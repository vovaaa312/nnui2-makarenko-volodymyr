...
with open("../docs/experiment_01.md", "w", encoding="utf-8") as f:
    f.write("# Záznam do deníku – Perceptron a Wine dataset\n\n")
    f.write("## Popis úlohy\n")
    f.write("Cílem experimentu bylo implementovat jednoduchý perceptron pro binární klasifikaci dat z datasetu **Wine**. Úkolem bylo rozlišit třídu 1 od ostatních tříd (spojených do třídy 0). Dataset obsahuje 178 vzorků s 13 příznaky.\n\n")
    f.write("## Parametry trénování\n")
    f.write("- **Dataset**: [Wine (UCI)](https://archive.ics.uci.edu/dataset/186/wine)\n")
    f.write("- **Počet trénování**: 10\n")
    f.write("- **Počet epoch**: 100\n")
    f.write("- **Learning rate**: 0.1\n")
    f.write("- **Aktivační funkce**: Kroková funkce (0/1)\n")
    f.write("- **Rozdělení dat**: 80 % trénink, 20 % test (142 vzorků trénink, 36 vzorků test)\n")
    f.write("- **Normalizace vstupů**: StandardScaler()\n\n")
    f.write("## Výsledky experimentu\n")
    f.write(f"- **Nejlepší model**: Běh č. {best_idx}\n")
    f.write(f"- **Přesnost na testovací množině**: {best_accuracy*100:.1f} %\n")
    f.write("- **Váhy nejlepšího modelu**:\n")
    f.write("```python\n")
    f.write(f"{best_weights}\n")
    f.write("```\n")
    f.write("- **Přesnost jednotlivých běhů**:\n")
    f.write("| Běh | Přesnost (%) |\n")
    f.write("|-----|--------------|\n")
    for i, acc in enumerate(all_accuracies):
        f.write(f"| {i}   | {acc*100:.1f}        |\n")
    f.write("\n### Boxplot trénovacích chyb (posledních 10 epoch)\n")
    f.write("![Boxplot trénovacích chyb](images/perceptron_boxplot.png)\n")